...
//uint64_t randomId = (((uint64_t)RandomInt(0, 0xFFFFFFFF)) << 32) | RandomInt(0, 0xFFFFFFFF);
CGCClientSharedObjectTypeCache* socache;
 ...
 */
CEconItemView* CreateAndAddEconItemView(CPlayerInventory* inventory, uint32 itemDefIdx) {
	CEconItemView* econItemView = new CEconItemView();
	econItemView->Init(itemDefIdx, AE_USE_SCRIPT_VALUE, AE_USE_SCRIPT_VALUE, inventory->GetOwner().GetAccountID());

	CEconItem* econItem = new CEconItem();
	econItem->SetAccountID(econItemView->GetAccountID());
	econItem->SetDefinitionIndex(econItemView->GetItemDefIndex());

	int32 a = econItemView->GetQuality();
	econItem->SetQuality(a == AE_NORMAL ? AE_UNIQUE : a);

	econItem->SetOrigin(eEconItemOrigin::kEconItemOrigin_Drop);
	econItem->SetItemLevel(econItemView->GetItemDefinition()->RollItemLevel());
	...
	econItem->SetItemID(itemDefIdx);
	econItem->SetQuantity(1);

	// the big boy of the operation that fixes a crash
	econItemView->SetNonSOEconItem(econItem);

	// the main thing that makes the item exist
	inventory->GetSOC()->FindTypeCache(CEconItem::k_nTypeID)->AddObject(econItemView->GetSOCData());

	Msg("Adding economy item %s through SOCache for %s.\n", econItem->GetItemDefinition()->GetItemDefinitionName(), SteamFriends()->GetFriendPersonaName(inventory->GetOwner()));
	return econItemView;
}
...
void CPlayerInventory::SOCacheSubscribed(const CSteamID& steamIDOwner, GCSDK::ESOCacheEvent eEvent)
{
	...
	//if (eEvent == eSOCacheEvent_Subscribed ) 
	{
		// This creates the custom weapon with item definition index 1200
		auto pSchema = GetItemSchema();
		if (!pSchema) return;

		GameItemDefinition_t* pItemDef1 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(1155));
		GameItemDefinition_t* pItemDef2 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(1154));
		GameItemDefinition_t* pItemDef3 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(1161));
		GameItemDefinition_t* pItemDef4 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(1027));
		GameItemDefinition_t* pItemDef5 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(928));
		GameItemDefinition_t* pItemDef6 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(463));
		GameItemDefinition_t* pItemDef7 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(1110));
		GameItemDefinition_t* pItemDef8 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(640));
		GameItemDefinition_t* pItemDef9 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(162));
		GameItemDefinition_t* pItemDef10 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(169));
		GameItemDefinition_t* pItemDef11 = static_cast<GameItemDefinition_t*>(pSchema->GetItemDefinition(902));

		if (pItemDef1)
			CreateAndAddEconItem(this, pItemDef1);
		if (pItemDef2)
			CreateAndAddEconItem(this, pItemDef2);
		if (pItemDef3)
			CreateAndAddEconItem(this, pItemDef3);
		if (pItemDef4)
			CreateAndAddEconItem(this, pItemDef4);
		if (pItemDef5)
			CreateAndAddEconItem(this, pItemDef5);
		if (pItemDef6)
			CreateAndAddEconItem(this, pItemDef6);
		if (pItemDef7)
			CreateAndAddEconItem(this, pItemDef7);
		if (pItemDef8)
			CreateAndAddEconItem(this, pItemDef8);
		if (pItemDef9)
			CreateAndAddEconItem(this, pItemDef9);
		if (pItemDef10)
			CreateAndAddEconItem(this, pItemDef10);
		if (pItemDef11)
			CreateAndAddEconItem(this, pItemDef11);
	}
